...
> Unlike some default names generated by Excel, the name _ThisWorkbookDataModel_ is the same whatever the region langage set for the user.
 ...
 It appears in the connection tab of the `Queries & Connections` panel, but is not always relevant, as it may remain displayed even after it gets empty, or not display while containing data, as it is explained why below.
...
This also explains why in the `Queries & Connections` panel :
