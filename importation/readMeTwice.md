...
> Unlike some default names generated by Excel, the name _ThisWorkbookDataModel_ is the same whatever the region langage set for the user.
 ...
 It appears in the connection tab of the `Queries & Connections` panel, but is not always relevant :
 - It never appears when document starts, whenever some connection tables exist or not.
 - It sometimes stucks after everything have been deleted, and it is removable by hand-loading the last present data as connection out of the model object. Also it still persists as name after that in the connections collection of the Excel Object Model for Vba.

> [!CAUTION]
> This is the first observation I made, which is explained by the split design of Excel. It seems first that components not storing metadata in the same way or in the same the part of the document. There are equivalent metadata and components try to affect every one of them, but it is not constant process. It explains then persisting objects and fantoms.

This also explains why in the last panel :
